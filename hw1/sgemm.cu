...
void basicSgemm_d_1thread1row(int m, int k, int n, const float *A_h, const float *B_h, float* C_h) {
    printf("Device Matrix Multiplication: 1 Thread-1 Row\n");
    float *A_d, *B_d, *C_d;
    allocDeviceMatrices(m, k, n, &A_d, &B_d, &C_d);
    copyToDevice(m, k, n, A_h, B_h, A_d, B_d);

    long t0 = myCpuTimer();
    int gridSize = (int)ceil((float)m/BLOCK_SIZE);
    matrixMulKernel_1thread1row<<<gridSize, BLOCK_SIZE>>>(m, k, n, A_d, B_d, C_d);
    CHECK(cudaDeviceSynchronize());
    printTimeDelta("\tKernel Execution", t0, myCpuTimer());
...
void basicSgemm_d_1thread1col(int m, int k, int n, const float *A_h, const float *B_h, float* C_h) {
    printf("Device Matrix Multiplication: 1 Thread-1 Column\n");
    float *A_d, *B_d, *C_d;
    allocDeviceMatrices(m, k, n, &A_d, &B_d, &C_d);
    copyToDevice(m, k, n, A_h, B_h, A_d, B_d);
    ...
    long t0 = myCpuTimer();
    matrixMulKernel_1thread1col<<<gridSize, BLOCK_SIZE>>>(m, k, n, A_d, B_d, C_d);
    CHECK(cudaDeviceSynchronize());
    printTimeDelta("\tKernel Execution", t0, myCpuTimer());
...
int main(int argc, char const *argv[]) {
    ...
    float *C_h_1t1r = (float*)malloc(m*n*sizeof(float));
    float *C_h_1t1c = (float*)malloc(m*n*sizeof(float));
    ...
    basicSgemm_d_1thread1row(m, k, n, A_h, B_h, C_h_1t1r);
    basicSgemm_d_1thread1col(m, k, n, A_h, B_h, C_h_1t1c);
    ...
    if(matEq(m, n, C_h, C_h_1t1r)) {
        printf("1 Thread 1 Row: Correct\n");
    } else {
        printf("1 Thread 1 Row: Incorrect\n");
    }
    if(matEq(m, n, C_h, C_h_1t1c)) {
        printf("1 Thread 1 Column: Correct\n");
    } else {
        printf("1 Thread 1 Column: Incorrect\n");
    }
